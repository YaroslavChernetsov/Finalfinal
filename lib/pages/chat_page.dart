...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _initializeChatPage() async {
    try {
      ...
      if (isAdmin!) {
        ...
        print('[DEBUG] Загружаем чат с магазином...');
        await _loadUserChat();
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (isAdmin!) {
      return Scaffold(
        appBar: AppBar(title: const Text('Чаты с клиентами')),
        body: chats.isEmpty
...
class ChatDetailPage extends StatelessWidget {
  ...
  const ChatDetailPage({Key? key, required this.chatId, required this.currentUserId})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Чат с магазином носков')),
      body: Column(
        children: [
          Expanded(
